  ...
  return (
    <Tabs.Root defaultValue={sections[0].id}>
      <Tabs.List className="relative z-0 mt-5 flex w-full rounded-full border border-line bg-surface p-1">
        <Tabs.Indicator
          className="absolute top-1 bottom-1 left-0 -z-1 rounded-full bg-accent transition-all duration-200 ease-out"
          style={{
      ...
      {sections.map((s) => (
        <Tabs.Panel key={s.id} value={s.id}>
          <Section section={{ ...s, description: s.description }} hideHead={false} />
        </Tabs.Panel>
      ))}
    </Tabs.Root>
  );
